...
COPY --from=build /target/demo-0.0.1-SNAPSHOT.jar line-login-starter.jar//前のビルドステージからビルドされたjarファイル(demo.jar)を新しいビルドステージにコピー
EXPOSE 8080//ポート8080を宣言
ENTRYPOINT ["java", "-jar", "line-login-starter.jar"]//実行するファイル(demo.jar)を指定
